...
class BinaryTree(Generic[T]):
    ...
    def __contains_helper(self, node: 'BinaryNode[T]', value: T):
        if node is None:
            return False
    ...
    def add(self, value: T):
        ...
        if self.__root is None:
            self.__root = BinaryNode[T](value)
        else:
            self.__add_helper(self.__root, value)
        self.__weight += 1

    def __add_helper(self, node: 'BinaryNode[T]', value: T):
        if value == node.value:
    ...
    def __remove_helper(self, node: 'BinaryNode[T]', value: T):
        if node is None:
            return None
        elif value < node.value:
    ...
    def __find_min(self, node: 'BinaryNode[T]'):
    ...
    def __in_order_traversal_helper(self, node: 'BinaryNode[T]', values: [ComparableValue]):
        if node is not None:
    ...
    def __post_order_traversal_helper(self, node: 'BinaryNode[T]', values: [ComparableValue]):
        if node is not None:
    ...
    def __pre_order_traversal_helper(self, node : 'BinaryNode[T]', values: [ComparableValue]):
        if node is not None:
            values.append(node.value)
...
class DoubleLinkedList(Generic[T]):
    ...
    def add(self, index: int, value: T):
        ...
        new_node = DoubleLinkedNode[T](value)
        if index == 0:
    ...
    def resize(self):
        ...
        for node in self.table:
            while node is not None:
                ...
                new_node = HashNode(node.key, node.value)
...
class LinkedList(Generic[T]):
    ...
    def append(self, value: T):
        if self.isEmpty():
            self._head = LinkedNode[T](value)
        else:
            current = self._head
            while current.next:
                current = current.next
            current.next = LinkedNode[T](value)
        self._size += 1
        self._sorted = False
    ...
    def _merge_sort(self, head: 'LinkedNode'):
    ...
    def _sorted_merge(self, a: 'LinkedNode', b: 'LinkedNode'):
        if not a:
            return b
        if not b:
            return a
    ...
    def _get_middle(self, head: 'LinkedNode'):
        if not head:
            return head
        slow = head
        fast = head
